...
const int NODES = 3;            // Number of nodes in the network
const int CLASSES = 2;          // Number of different classes

double routing[CLASSES][NODES][NODES];
vector< handle< server > > nodes;
...
public:
    server( const string& name, int id, const vector<double>& servTime );
    list< job* > queue;
    list< handle<server> > blocked; // blocked servers
protected:
    void inner_body( void );
    const int           _id;
    vector<double>      _servTime;
    rngDiscEmpirical    _next;
};
...
public:
    source( const string& name, int id, int c, double r ) : 
	process( name ),
        _id( id ),
        _c( c ),
        _r( new rngNegExp( name+"_rng", r ) { };
protected:
    void inner_body( void );
    int         _id;
    int         _c;
...
server::server( const string& name, int id, const vector<double>& servTime ) :
    process( name ),
    _id( id ),
    _servTime( servTime )
    { 
...
void server::inner_body( void )
{
    ...
    while ( 1 ) {
        ...
        queue.pop_front( );
        // Serve the job
        hold( servTime[ j->_c ]->value() );
        // Now, check if the destination server's queue is not full
...
int main( int argc, char* argv[] )
{
    ...
    if ( numRep > 1 ) {
        ...
        // wtime   = new jain( "Waiting_Time" , .95 );
        // tput    = new jain( "Throughput", .95 );
    } else {
	// wtime           = new bmeans( "Waiting_Time", confp, 512 );
	// tput            = new bmeans( "Throughput", confp, 512 );
        wtime           = new bmeans2( "Waiting_Time", confp );
        tput            = new bmeans2( "Throughput", confp );
    }
    completed           = new counter( "Completed" );
    job_inter_time      = new rngExp( "Job Interarr. Time", 0.09 );
    job_serv_time       = new rngExp( "Service Time", 0.1 );
